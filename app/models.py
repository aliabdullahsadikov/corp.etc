...
class Comment(db.Model):
    ...
    def get_time(self):
        ...
        if days > 0:
            return self.get_str(days, 'день') if days == 1 else self.get_str(days, 'дней')
        elif hour > 0:
            return self.get_str(hour, 'час') if hour == 1 else self.get_str(hour, 'часов')
        elif min > 0:
            return self.get_str(min, 'минута') if min == 1 else self.get_str(min, 'минут')


    def get_str(self, number, format):
        return f"{number}  {format} назад"
